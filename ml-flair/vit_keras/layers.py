...
class MultiHeadSelfAttention(tf.keras.layers.Layer):
    def __init__(self, *args, num_heads, learnable_params, **kwargs):
    ...
    def build(self, input_shape):
        ...
        self.query_dense = tf.keras.layers.Dense(
            hidden_size,
            name="query",
            trainable=False if 'film' in self.learnable_params else True
        )
...
class TransformerBlock(tf.keras.layers.Layer):
    ...
    def __init__(self, *args, num_heads, mlp_dim, dropout, learnable_params, **kwargs):
        super().__init__(*args, **kwargs)
        self.num_heads = num_heads
    ...
    def build(self, input_shape):
        ...
        if 'film' in self.learnable_params:
            self.mlpblock.trainable = False

        self.layernorm1 = tf.keras.layers.LayerNormalization(
            epsilon=1e-6, name=f"{self.name}/LayerNorm_0"
        )
        self.layernorm2 = tf.keras.layers.LayerNormalization(
            epsilon=1e-6, name=f"{self.name}/LayerNorm_2"
        )
